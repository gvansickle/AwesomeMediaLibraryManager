...
public:
	...
	{
		qDebug() << M_THREADNAME() << "future state:" << state();
	}
...
{
	qDebug() << M_THREADNAME() << "WAIT START, future state:" << state();
	//waitForFinished();
	this->results();
	qDebug() << M_THREADNAME() << "WAIT COMPLETE, future state:" << state();
}
...
QDebug operator<<(QDebug dbg, const ExtFuture<T> &extfuture)
{
	...
	dbg << M_THREADNAME() << "ExtFuture<T>(" /*<< extfuture.future()*/ << "STATE:" << extfuture.state() << ")";
